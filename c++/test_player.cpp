...
void test_player() {
	...
	test.add_test(test.assert_true(
		"Test whether after put under the following cards in this order [(2, swords),(6, clubs),(3, cups)] then these should be put under in the reversed order if the specification \"reversed\" is put in the function call",
		"The cards from putUnder were not put in the reverse order",
		[&player] {
			std::vector<Card> cards_to_put_under{ Card(2, "swords"), Card(6, "clubs"), Card(3, "cups") };
			player.putUnder(cards_to_put_under, "reversed");

			return ((player.cards.at(0) == Card(3, "cups")) && (player.cards.at(1) == Card(6, "clubs")) && (player.cards.at(2) == Card(2, "swords")));
		}
	));
